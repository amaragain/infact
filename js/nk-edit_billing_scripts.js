...
/*	$("#billgeneratingTableBody").on('focus','.product-input-field', function () {
	...
	function addBillingRow(){
		...
		newRow +='	</tr><input class="hidden-id-input" type="hidden" data-row-count-id="'+rowCountIncrement+'" />';
	...
	function saveBill(){
		...
		$(".billing-rows").each(function(){
		    ...
		    billItems[z] = {uniqueid:prdUniqueId, title:prdName, tax:prdTax, qty:prdQuantity, price:prdPrice}
	...
	$(document).on('change', '.type-change-field', function() {
	    if(this.checked) {
	        //Do stuff
	        $('.bill-type-info').text('Tax Invoice [B2B] - Cash');
	        $('.cust-gst-box').slideDown();
	    }
	    else {
	        //Do stuff
	        $('.bill-type-info').text('Tax Invoice [B2C] - Cash');
