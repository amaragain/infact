...
$(document).ready(function(){
	...
	function addBillingRow(){
		...
		newRow +='		<td class="text-center billcol-qty"><input class="form-control form-pad-6 qty-input-field" data-row-count-prdqty="'+rowCountIncrement+'" data-row-prd-rem="" type="number" min="1" value="1" /></td>';
	...
	function updateThisRow(dataRowCount){
		...
		var curPrdRemaining = $("[data-row-count-prdqty="+dataRowCount+"]").attr('data-row-prd-rem');

		if(parseInt(curPrdRemaining) < parseInt(prdQty)){alert("Insufficiernt stock. Only "+curPrdRemaining+" remaining.");}
	        ...
	        if($(":input.hidden-id-input[value='"+productId+"']").length > 0){
		        ...
		        $.post(
		        	...
		        	function(data){
		        		...
		        		//priceWithoutTax = (100 * priceWithTax) / (100 + tax)
		        		var productTaxValue 	= jsonData.taxValue;
		        		var productPrice 		= (100 * jsonData.prdPrice) / (Number(productTaxValue) + 100);
		        		var productTaxTitle 	= jsonData.taxTitle;
		        		var productRemaining 	= jsonData.stock;
						...
						$("[data-row-count-prdqty="+dataRowCount+"]").attr('data-row-prd-rem',productRemaining);
		        		$("[data-row-count-id="+dataRowCount+"]").val(productId);
		        		//alert(productId);
		        		//alert($("[data-row-count-id="+dataRowCount+"]").val());
	...
	function saveBill(){
        ...
        if($('.type-change-field').prop("checked") == true){
			...
			var billType 	 	= '2'; //   0 is for B2C bill
        }
		...
		$(".billing-rows").each(function(){
		    ...
		    //alert($('[data-row-count-id='+thisRowCount+']').val())
		    prodId 		= $('[data-row-count-id='+thisRowCount+']').val();
		    ...
		    billItems[z] = {id:prodId, uniqueid:prdUniqueId, title:prdName, tax:prdTax, qty:prdQuantity, price:prdPrice}
	...
	function fetchBillNo(billtype)
	{

	    $.post(
	    	'prg_fetch_bill_number.php',
	    	{
	    		type: billtype
	    	},
	    	function(data){
	    		//alert(data)
	    		if (billtype == 1){ billPrefix = "S";} 
	    		else{ billPrefix = "SE";} 
	    		$('.bill-no-cell').html(billPrefix + ' <span id="billNo">' + data + '</span>');
	    	}
	    );
	}
