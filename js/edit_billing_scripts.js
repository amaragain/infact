...
$(document).ready(function(){
	...
	function addBillingRow(){
		...
		newRow +='		<td class="text-center billcol-qty"><input class="form-control form-pad-6 qty-input-field" data-row-count-prdqty="'+rowCountIncrement+'" type="number" min="1" value="1" /></td>';
	...
	function getTotal() {
		...
		$("[data-row-count-netamt]").each(function(){
		    sumNetAmt += parseFloat($(this).text().replace(",",""));  // Or this.innerHTML, this.innerText
		});
		...
		$("[data-row-count-taxamt]").each(function(){
		    sumTaxAmt += parseFloat($(this).text().replace(",",""));  // Or this.innerHTML, this.innerText
		});
		...
		$("[data-row-count-prdttl]").each(function(){
		    sumTtlAmt += parseFloat($(this).text().replace(",",""));  // Or this.innerHTML, this.innerText
		});
	...
	function saveBill(){
		...
		var billNumber		= $('#billNo').text();
		var billNo 			= $('.bill-id-hidden').val();
	...
	function taxTable(){
		...
		$('.tax-table-row').each(function(){
			...
			$('[data-this-tax-val="'+taxRowNumber+'"]').each(function(){
			    taxableSum += parseFloat($(this).text().replace(",",""));  // Or this.innerHTML, this.innerText
			});
