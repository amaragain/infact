...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search-string'])) {

	$search_by 			= $_POST['search-by'];
	$search_string 		= $_POST['search-string'];

	$search_param = "`".$search_by."` LIKE '%".$search_string."%'";

	if($search_by == 'stock' || $search_by == 'buy_price' || $search_by == 'sell_price'){$search_param = "`".$search_by."` < ".$search_string;}


	$fetch_prd_query = "SELECT * FROM `products` WHERE ".$search_param." ORDER By `title` ASC;";
